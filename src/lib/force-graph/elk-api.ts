...
export class ElkApi {
	...
	private id = 0;
	private readonly resolvers: Record<number, (error: any, result?: any) => void> = {};
	...
	constructor() {
		...
		this.worker.onmessage = (answer: MessageEvent<{id: number; error?: any; data?: any}>) => {
	...
	private async run(cmd: string, options: Record<string, any> = {}): Promise<any> {
		const id = this.id++;
